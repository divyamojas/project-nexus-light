...
export function DashboardContent({
  ...
  const { data: allBooks, mutate: mutateBooks } = useSWR('/books', fetcher, { fallbackData: initialBooks })
  const { data: incoming, mutate: mutateIncoming } = useSWR('/requests/incoming', fetcher, { fallbackData: initialIncoming })
  const { data: outgoing, mutate: mutateOutgoing } = useSWR('/requests/outgoing', fetcher, { fallbackData: initialOutgoing })
  const { data: transfers, mutate: mutateTransfers } = useSWR('/transfers', fetcher, { fallbackData: initialTransfers })
  const { data: loans, mutate: mutateLoans } = useSWR('/loans', fetcher, { fallbackData: initialLoans })

  // Seed the global SWR cache with server-fetched data so navigating to
  // /books, /library etc. finds data already cached — zero extra requests.
  // fallbackData is hook-local and doesn't write to the global cache itself.
  useEffect(() => {
    const seeds = [
      ['/books', initialBooks],
      ['/requests/incoming', initialIncoming],
      ['/requests/outgoing', initialOutgoing],
      ['/transfers', initialTransfers],
      ['/loans', initialLoans],
    ]
    seeds.forEach(([key, data]) => {
      if (data?.length) globalMutate(key, data, { revalidate: false })
    })
  ...
  const myBooks = (allBooks || []).filter(b => b.user_id === pid)
  const browseBooks = (allBooks || []).filter(b => b.status === 'available' && !b.archived && b.user_id !== pid).slice(0, 8)
    ...
    try {
      if (type === 'archive' || type === 'unarchive') {
        ...
        const ok = await requestConfirm({
          ...
          subtitle: 'Permanent — gone from Leaflet.',
          message: `Deleting "${title}" removes it from the platform. Any pending requests for this book will be cancelled. Your lending history stays intact.`,
          mode: 'countdown',
          countdown: 2,
          challenge: 'Last chance to change your mind…',
  ...
  const libraryBorrows = (loans || []).filter(l => l.borrower_id === pid && l.status === 'active' && l.book_source === 'library')
  const p2pLoans = (loans || []).filter(l => l.book_source !== 'library')
  const lentLoans = p2pLoans.filter(l => l.lender_id === pid && l.status === 'active')
  const borrowedLoans = p2pLoans.filter(l => l.borrower_id === pid && l.status === 'active')
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-10">
            ...
            {incoming.map(req => (
              <div key={req.id} className="card p-4 flex items-center justify-between gap-4">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => handleRequestAction('accepted', req.id)} className="btn-primary text-xs py-1.5 px-3">Accept</button>
                  <button onClick={() => handleRequestAction('rejected', req.id)} className="btn-danger text-xs py-1.5 px-3">Reject</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {(outgoing || []).length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">Outgoing Requests</h2>
          <div className="space-y-3">
            {outgoing.map(req => (
              <div key={req.id} className="card p-4 flex items-center justify-between gap-4">
                <div className="min-w-0">
                ...
                {req.status === 'pending' && (
                  <button onClick={() => handleRequestAction('cancelled', req.id)} className="btn-secondary text-xs py-1.5 px-3 flex-shrink-0">Cancel</button>
                )}
              </div>
            ))}
          </div>
      ...
      {(transfers || []).length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">Active Transfers</h2>
          <div className="space-y-3">
            {transfers.map(tr => (
              <div key={tr.id} className="card p-4 flex items-center justify-between gap-4">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  {tr.status === 'pending' && (
                    <button onClick={() => handleTransferAction('confirmed', tr.id)} className="btn-primary text-xs py-1.5 px-3">Confirm</button>
                  )}
                  {tr.status === 'confirmed' && (
                    <button onClick={() => handleTransferAction('complete', tr.id)} className="btn-primary text-xs py-1.5 px-3">Complete</button>
                  )}
                </div>
              </div>
            ))}
      ...
      {libraryBorrows.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">Library Borrows</h2>
          <div className="space-y-3">
            {libraryBorrows.map(loan => (
              <LibraryLoanCard key={loan.id} loan={loan} toast={toast} onRefresh={mutateLoans} />
            ))}
          </div>
        </section>
      )}
      ...
      {(lentLoans.length > 0 || borrowedLoans.length > 0) && (
        <section>
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">Active Loans</h2>
            ...
            {lentLoans.length > 0 && (
              <div>
                ...
                <div className="space-y-2">
                  {lentLoans.map(loan => (
                    <LoanCard key={loan.id} loan={loan} type="lent" toast={toast} onRefresh={mutateLoans} />
                  ))}
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {borrowedLoans.map(loan => (
                    <LoanCard key={loan.id} loan={loan} type="borrowed" toast={toast} onRefresh={mutateLoans} />
                  ))}
                </div>
              </div>
            )}
      ...
      <section>
        ...
        <BookGrid books={browseBooks} onAction={handleBookAction} currentUserId={pid} />
      </section>
      ...
      <section className="pb-4">
        <div className="card p-6 max-w-lg">
          ...
          />
          <button onClick={sendFeedback} disabled={sendingFeedback} className="btn-primary text-sm">
...
function LibraryLoanCard({ loan, toast, onRefresh }) {
  ...
  async function returnToLibrary() {
    setLoading(true)
    try {
      await apiFetch('/returns', { method: 'POST', body: JSON.stringify({ book_id: loan.book_id }) })
  ...
  return (
    <div className="card p-3 flex items-center justify-between gap-3 border-l-2 border-cyan-400 dark:border-cyan-600">
      <div className="min-w-0">
        <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{loan.title || 'Untitled'}</p>
        <p className="text-xs text-stone-500 dark:text-slate-400">
          📍 {loan.library_location || loan.library_name || 'Office Library'} · Borrowed {formatDate(loan.loaned_at)}
        </p>
      </div>
      <button onClick={returnToLibrary} disabled={loading} className="text-xs text-cyan-600 dark:text-cyan-400 hover:underline flex-shrink-0 disabled:opacity-50 whitespace-nowrap">
...
function LoanCard({ loan, type, toast, onRefresh }) {
  ...
  return (
    <div className="card p-3 flex items-center justify-between gap-3">
      <div className="min-w-0">
        <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{loan.book?.catalog?.title || 'Untitled'}</p>
        <p className="text-xs text-stone-500 dark:text-slate-400">
          {type === 'lent' ? `Borrowed by ${loan.borrower_name || 'user'}` : `Lent by ${loan.lender_name || 'user'}`}
        </p>
      </div>
      {type === 'borrowed' && !loan.return_request_id && (
        <button onClick={requestReturn} disabled={loading} className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline flex-shrink-0 disabled:opacity-50">
          Request Return
        </button>
      )}
